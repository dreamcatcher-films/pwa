...
const SideMenu: React.FC<SideMenuProps> = ({ isOpen, onClose }) => {
    ...
    return (
        <>
            {/* Overlay */}
            <div 
                className={`fixed inset-0 bg-black z-50 transition-opacity duration-300 ${isOpen ? 'bg-opacity-50' : 'bg-opacity-0 pointer-events-none'}`}
                onClick={onClose}
                aria-hidden="true"
            ></div>
            ...
            {/* Side Menu */}
            <div 
                className={`fixed top-0 left-0 h-full bg-[#0F3E34] w-72 shadow-xl z-60 flex flex-col transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
                role="dialog"
                aria-modal="true"
            >
