...
import { EngagementRingSpinner, PlusCircleIcon, PencilSquareIcon, TrashIcon, XMarkIcon, EnvelopeIcon, InformationCircleIcon } from '../Icons.tsx';
import {
    getAdminGuests, addAdminGuest, updateAdminGuest, deleteAdminGuest,
    getAdminGuestGroups, addAdminGuestGroup, deleteAdminGuestGroup,
    sendAdminGuestInvites
} from '../../api.ts';
...
// --- COMPONENTS ---
const GuestEditorModal: FC<{ guest?: Guest; groups: GuestGroup[]; onClose: () => void; bookingId: string; }> = ({ guest, groups, onClose, bookingId }) => {
    const queryClient = useQueryClient();
    const { register, handleSubmit, watch } = useForm<GuestFormValues>({
        defaultValues: { name: guest?.name || '', email: guest?.email || '', group_id: guest?.group_id?.toString() || '', allowed_companions: guest?.allowed_companions || 0, rsvp_status: guest?.rsvp_status || 'pending' },
    });
    ...
    const mutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['adminGuests', bookingId] });
            onClose();
        },
    });
...
const AdminGuestManager: FC<{ bookingId: string }> = ({ bookingId }) => {
    ...
    const { data: guests, isLoading: isLoadingGuests, error: errorGuests } = useQuery<Guest[], Error>({ queryKey: ['adminGuests', bookingId], queryFn: () => getAdminGuests(bookingId) });
    ...
    const sendInvitesMutation = useMutation<{ message: string }, Error, void>({
        mutationFn: () => sendAdminGuestInvites({ bookingId }),
    ...
    const deleteGuestMutation = useMutation({
        ...
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['adminGuests', bookingId] }),
    });
    ...
    const addGroupMutation = useMutation({
        ...
        onSuccess: () => {
            setNewGroupName('');
            queryClient.invalidateQueries({ queryKey: ['adminGuestGroups', bookingId] });
        },
    });

    const deleteGroupMutation = useMutation({
        mutationFn: (groupId: number) => deleteAdminGuestGroup({ bookingId, groupId }),
        onSuccess: () => queryClient.invalidateQueries({ queryKey: ['adminGuestGroups', bookingId] }),
    });
    ...
    return (
        <div className="space-y-6">
            ...
            {feedbackMessage && (<div className={`p-3 rounded-lg text-sm ${feedbackMessage.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>{feedbackMessage.text}</div>)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex gap-4">
                            <button onClick={() => sendInvitesMutation.mutate()} disabled={!hasPendingGuests || sendInvitesMutation.isPending} className="flex items-center gap-2 bg-green-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-700 disabled:bg-green-300 disabled:cursor-not-allowed">
                                {sendInvitesMutation.isPending ? <EngagementRingSpinner className="w-5 h-5"/> : <EnvelopeIcon className="w-5 h-5"/>} Wyślij zaproszenia
                            </button>
                    ...
                    <div className="bg-white rounded-2xl shadow overflow-hidden">
                        <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="text-xs text-slate-700 uppercase bg-slate-50"><tr><th className="px-6 py-3 text-left">Gość</th><th className="px-6 py-3 text-left">Uwagi gościa</th><th className="px-6 py-3 text-center">Status</th><th className="px-6 py-3 text-right">Akcje</th></tr></thead><tbody>{filteredGuests?.map(guest => (<tr key={guest.id} className="border-b hover:bg-slate-50"><td className="px-6 py-4"><p className="font-medium text-slate-900">{guest.name}</p><p className="text-xs text-slate-500">{guest.group_name || 'Brak grupy'}</p></td><td className="px-6 py-4 text-slate-600 text-xs">{guest.notes ? <div className="flex items-start gap-2"><InformationCircleIcon className="w-4 h-4 mt-0.5 text-slate-400 flex-shrink-0"/><span>{guest.notes}</span></div> : <span className="italic text-slate-400">Brak</span>}</td><td className="px-6 py-4 text-center"><span className={`px-2 py-1 text-xs font-semibold rounded-full ${statusMap[guest.rsvp_status].color}`}>{statusMap[guest.rsvp_status].text}</span></td><td className="px-6 py-4 text-right"><button onClick={() => handleEdit(guest)} className="p-2 text-slate-500 hover:text-indigo-600"><PencilSquareIcon className="w-5 h-5"/></button><button onClick={() => handleDelete(guest.id)} className="p-2 text-slate-500 hover:text-red-600"><TrashIcon className="w-5 h-5"/></button></td></tr>))}</tbody></table>{filteredGuests?.length === 0 && <p className="text-center text-slate-500 py-8">Brak gości pasujących do filtra.</p>}</div>
                    </div>
                </div>
            ...
            {isModalOpen && <GuestEditorModal guest={editingGuest || undefined} groups={groups || []} onClose={() => setIsModalOpen(false)} bookingId={bookingId} />}
        </div>
    );
};
