...
const AdminPackagesPage: FC = () => {
    ...
    const renderContent = () => {
        ...
        switch (activeTab) {
            ...
            case 'addons': return <AddonsManager addons={addons} onDataChange={fetchData} token={token} />;
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setEditingPackage({ id: 0, name: '', description: '', price: 0, addons: [], category_id: null, is_published: false, rich_description: '', rich_description_image_url: '' })} className="flex items-center gap-2 bg-indigo-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-indigo-700">
...
// --- ADDONS MANAGER ---
const AddonsManager: FC<{ addons: Addon[], onDataChange: () => void, token: string | null }> = ({ addons, onDataChange, token }) => {
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setEditingAddon({})} className="flex items-center gap-2 bg-indigo-600 text-white font-bold py-2 px-4 rounded-lg hover:bg-indigo-700">
            ...
            {editingAddon && <SimpleEditor item={editingAddon} onClose={() => setEditingAddon(null)} onSave={handleSave} title="Dodatek" fields={[{ name: 'name', label: 'Nazwa' }, { name: 'price', label: 'Cena', type: 'number' }]} />}
        </div>
    );
};
...
// --- SIMPLE EDITOR MODAL (for Addons & Categories) ---
...
const PackageEditor: FC<{ pkg: Package, allAddons: Addon[], allCategories: Category[], onClose: () => void, onSave: () => void, token: string | null }> = ({ pkg, allAddons, allCategories, onClose, onSave, token }) => {
    ...
    const handleAddonToggle = (addonId: number, isLocked: boolean) => {
        const isIncluded = formData.addons.some(a => a.id === addonId);
        if (isIncluded) {
            setFormData(p => ({...p, addons: p.addons.filter(a => a.id !== addonId)}));
        } else {
            setFormData(p => ({...p, addons: [...p.addons, { id: addonId, is_locked: isLocked }]}));
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const endpoint = formData.id ? `/api/admin/packages/${formData.id}` : '/api/admin/packages';
                ...
                <div className="pt-4 border-t grid grid-cols-2 gap-6">
                    <div>
                        ...
                        <ul className="space-y-2 max-h-60 overflow-y-auto pr-2 border rounded-lg p-2">
                            {allAddons.filter(a => !includedAddonIds.has(a.id)).map(addon => (
                                <li key={addon.id} className="flex justify-between items-center p-2 rounded-md hover:bg-slate-50">
                                    ...
                                    <button type="button" onClick={() => handleAddonToggle(addon.id, false)} className="p-1 text-green-500 hover:text-green-700"><PlusCircleIcon/></button>
                                </li>
                            ))}
                        </ul>
                    </div>
                                ...
                                return (
                                    <li key={includedAddon.id} className="flex justify-between items-center p-2 bg-indigo-50 rounded-md">
                                        <label className="flex items-center gap-2 text-sm cursor-pointer">
                                            <input type="checkbox" checked={includedAddon.is_locked} onChange={() => handleAddonToggle(includedAddon.id, !includedAddon.is_locked)} className="rounded" />
                                            Zablokowany?
                                        </label>
                                        <span>{addonDetails.name}</span>
                                        <button type="button" onClick={() => handleAddonToggle(includedAddon.id, false)} className="p-1 text-red-500 hover:text-red-700"><TrashIcon/></button>
                                    </li>
                                );
                            })}
                        </ul>
